...
@login_required
def index(REQUEST):
    projects = Project.objects.filter(archived=False).filter(site=SITE_ID).filter(user=REQUEST.user)
    entries = Entry.objects.filter(project__in=[x.id for x in projects]).filter(user=REQUEST.user)
...
def submit(REQUEST, command = None):
    ...
    command_patterns = {
        'add_project': re.compile('^add (?P<project>.+)(?: with )?(?P<tags>.+)?$')
    }
